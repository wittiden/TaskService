...
class TestCreateUserCase:
    ...
    @pytest.mark.parametrize('is_standard, is_admin, is_vip', [(True, False, False), (False, True, False), (False, False, True)], ids=['standard', 'admin', 'vip'])
    @pytest.mark.unit
    @pytest.mark.asyncio
    async def test_create_user_good(self, mocker: MockerFixture, mock_user_commands, create_user_mock_case, is_standard, is_admin, is_vip):
        user = UsersFactory(admin=is_admin, vip=is_vip)

        mock_hash_pass = mocker.patch('app.modules.users.service.use_cases.hash_pass', return_value='hashed_password')
        ...
        result = await create_user_mock_case._create(user.name, user.email, user.password_hash, user.role)

        mock_hash_pass.assert_called_once_with(user.password_hash)
        mock_user_commands.insert_user_data.assert_awaited_once_with(user.name, user.email, 'hashed_password', user.role)
...
class TestUpdateUserCase:
    ...
    async def test_update_user_params_good(
        self, mocker: MockerFixture, mock_user_commands, new_params, mock_current_user_redis_commands, update_user_mock_case, mock_create_user_audit_case
    ):
        user = UsersFactory()

        if new_params is None:
            result = await update_user_mock_case.update_user_params(FullUserInfoDTO.model_validate(user), new_params)
            assert isinstance(result, SecurityUserInfoDTO)
            ...
            mock_current_user_redis_commands.set_current_user.assert_awaited_once_with(FullUserInfoDTO.model_validate(user_model))
...
class TestDeleteUserCase:
    ...
    @pytest.mark.unit
    @pytest.mark.asyncio
    async def test_close_my_account_good(self, mock_user_commands, mock_logout_user_case, delete_user_mock_case, mock_create_user_audit_case):
        ...
        mock_logout_user_case.logout_all_user_devices.assert_awaited_once_with(FullUserInfoDTO.model_validate(user))
    ...
    @pytest.mark.unit
    @pytest.mark.asyncio
    async def test_delete_user_account_good(self, mock_user_commands, mock_logout_user_case, delete_user_mock_case):
        user = UsersFactory(close=True)
...
class TestManageUserCase:
    ...
    @pytest.mark.unit
    @pytest.mark.asyncio
    async def test_block_user_good(self, mock_user_queries, mock_user_commands, mock_logout_user_case, manage_user_mock_case, mock_create_user_audit_case):
    ...
    @pytest.mark.unit
    @pytest.mark.asyncio
    async def test_unblock_user_good(self, mock_user_queries, mock_user_commands, manage_user_mock_case, mock_create_user_audit_case):
        user = UsersFactory(block=True)

...
def app_healthcheck_endpoint(request: Request) -> dict[str, str]:
    return application_health()


@health_router.get('/db', summary='Database healthcheck')
@inject
async def db_healthcheck_endpoint(request: Request, async_engine: FromDishka[AsyncEngine]) -> dict[str, str]:
    return await database_health(async_engine)


@health_router.get('/redis', summary='Redis healthcheck')
@inject
async def redis_healthcheck_endpoint(request: Request, redis_client: FromDishka[Redis]) -> dict[str, str]:

...
async def create_standard_endpoint(
    request: Request, response: Response, schema: CreateUserSchema, case: FromDishka[CreateUserCase], uow: FromDishka[UnitOfWork]
) -> SecurityUserInfoDTO:
...
async def update_me_endpoint(
    request: Request, response: Response, current_user: CurrentUser, schema: UpdateUserSchema, case: FromDishka[UpdateUserCase], uow: FromDishka[UnitOfWork]
) -> SecurityUserInfoDTO:
...
async def delete_user_account_endpoint(
    request: Request, response: Response, current_user: CurrentAdmin, user_id: UUID, case: FromDishka[DeleteUserCase], uow: FromDishka[UnitOfWork]
) -> None:
...
async def block_user_account_endpoint(
    request: Request, response: Response, current_user: CurrentAdmin, user_id: UUID, case: FromDishka[ManageUserCase], uow: FromDishka[UnitOfWork]
) -> FullUserInfoDTO:
...
async def show_users_endpoint(
    request: Request, response: Response, current_user: CurrentAdmin, case: FromDishka[ShowUserCase], uow: FromDishka[UnitOfWork], offset: int = 0, limit: int = 100

...
class UserCommandsRepository:
    ...
    async def alter_user_closed_param(self, user_id: UUID) -> datetime | None:
        result = await self._async_session.execute(
            update(UserModel).where(UserModel.user_id == user_id, UserModel.closed_at.is_(None)).values(closed_at=datetime.now(UTC)).returning(UserModel.closed_at)
        )

        return result.scalar_one_or_none()

    async def delete_closed_user_by_id(self, user_id: UUID) -> UUID | None:
        deleted_obj_id = await self._async_session.execute(delete(UserModel).where(UserModel.user_id == user_id, UserModel.closed_at.is_not(None)).returning(UserModel.user_id))
    ...
    async def alter_block_user_by_id(self, user_id: UUID) -> UserModel | None:
        user = await self._async_session.execute(
            update(UserModel).where(UserModel.user_id == user_id, UserModel.blocked_at.is_(None)).values(blocked_at=datetime.now(UTC)).returning(UserModel)
        )
    ...
    async def alter_unblock_user_by_id(self, user_id: UUID) -> UserModel | None:
        user = await self._async_session.execute(
            update(UserModel).where(UserModel.user_id == user_id, UserModel.blocked_at.is_not(None)).values(blocked_at=None).returning(UserModel)
        )
    ...
    async def alter_user_params(self, user_id: UUID, new_params: dict) -> UserModel | None:
        try:
            user = await self._async_session.execute(update(UserModel).where(UserModel.user_id == user_id).values(**new_params).returning(UserModel))

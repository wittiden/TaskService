...
@inject
async def login_user_endpoint(request: Request, response: Response, schema: LoginUserSchema, case: FromDishka[LoginUserCase], uow: FromDishka[UnitOfWork]) -> TokenInfoDTO:
...
@inject
async def logout_user_device_endpoint(request: Request, response: Response, current_user: CurrentUser, case: FromDishka[LogoutUserCase], uow: FromDishka[UnitOfWork]) -> None:
    await case.logout_user_device(current_user)


@auth_router.post('/logout-all', summary='Logout all user device', status_code=status.HTTP_204_NO_CONTENT)
@limiter.shared_limit('10/minute', scope='logout_limit')
@inject
async def logout_all_user_device_endpoint(request: Request, response: Response, current_user: CurrentUser, case: FromDishka[LogoutUserCase], uow: FromDishka[UnitOfWork]) -> None:
...
@inject
async def refresh_endpoint(request: Request, response: Response, schema: RefreshSchema, case: FromDishka[RefreshUserCase], uow: FromDishka[UnitOfWork]) -> TokenInfoDTO:
